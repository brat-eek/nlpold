...
def load_wv_raw(fname):
    return pd.read_table(fname, sep="\s+",
                         ...
                         index_col=0,
                         quoting=3)
#-------------------------creating the struct of - list of list( for each line) of tuples----------------------
...
def docs_to_windows(docs, word_to_num, tag_to_num, wsize=3): # docs is train data - (word | label)
    pad = (wsize - 1)/2
    docs = flatten1([pad_sequence(seq, left=pad, right=pad) for seq in docs])

    words, tags = zip(*docs)
    words = [canonicalize_word(w, word_to_num) for w in words]   # word filtering
